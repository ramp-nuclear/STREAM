...
class Pump(LumpedComponent):
    ...
    def __init__(self, pressure: Pascal | None = None,
        ...
        if pressure is not None and mdot0 is not None:
            raise ValueError(
                f"One cannot impose both pressure and current, "
                f"in this case you tried {pressure=}, {mdot0=}"
                )
        self.name = name
        self.p = pressure
        self.mdot0 = mdot0
    ...
    def calculate(self, variables: Sequence[float], *,
        ...
        if pressure is mdot0 is self.p is self.mdot0 is None:
            raise ValueError("One has to impose either the pressure or the flow "
                             "in a pump")
        out = np.empty(2)
        out[0] = variables[0] - self.T_out(Tin=directed_Tin(Tin, Tin_minus, mdot),
                                           mdot=mdot)
