...
def flow_graph_to_aggregator(
        f_graph: MultiDiGraph, funcs: ExternalFunctions | None = None
        ) -> CalculationGraph:
...
def flow_graph_to_agr_and_k(
    ...
    agr = kirchhoffify(
        agr, K, inertial_comps=inertial_comps, ref_mdots=ref_mdots or K.ref_mdots.keys(),
...
class FlowGraph:
    ...
    def __init__(self, *edges, funcs: ExternalFunctions | None = None,
                 reference_node: tuple[Hashable, Pascal] = None,
                 abs_pressure_comps: Sequence[Hashable] = None,
                 inertial_comps: Sequence[Calculation] = None,
                 ref_mdots: Sequence[Calculation] = None,
    ...
    def check_gravity_mismatch(self, temperature: Celsius = 10.0,
                               strategy: HydraulicStrategyMap | None = None,
                               tol: float = 1e-5, head: Pascal = 1.0) -> None:
