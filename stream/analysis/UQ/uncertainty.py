...
class Uncertainty:
    ...
    def as_absolute(self, nominal: Value) -> Uncertuple:
        ...
        """
        return (self.systematic_relative * nominal + self.systematic_absolute,
                np.sqrt((self.statistical_relative * nominal) ** 2
                        + self.statistical_absolute ** 2)
                )

    def __bool__(self):
        return any((self.systematic_relative, self.statistical_relative,
                    self.statistical_absolute, self.systematic_absolute))

    def __add__(self, other: "Uncertainty") -> "Uncertainty":
        if not isinstance(other, Uncertainty):
            return NotImplemented
        return Uncertainty(self.systematic_relative + other.systematic_relative,
                           np.sqrt(
                               self.statistical_relative ** 2 + other.statistical_relative ** 2),
                           self.systematic_absolute + other.systematic_absolute,
                           np.sqrt(
                               self.statistical_absolute ** 2 + other.statistical_absolute ** 2))
