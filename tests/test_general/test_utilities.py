...
decent_floats = st.floats(min_value=-1e20, max_value=1e20,
...
@given(arrays(float, st.integers(1, 10), elements=decent_floats),
       decent_floats,
       st.integers(0, 2))
...
def test_cosine_shape_achieves_maximum_at_center(start, length, ppf, n):
    ...
    assert (np.argmax(shape) == n) or np.isclose(ppf, 1.), shape
