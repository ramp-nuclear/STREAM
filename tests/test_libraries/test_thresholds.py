...
from stream.physical_models.thresholds import (
    Fabrega_CHF, Mirshak_CHF, Sudo_Kaminaga_CHF,
    Whittle_Forgan_OFI, Saha_Zuber_OSV, boiling_power)
...
def test_WF_OFI_for_precalculated_case(mdot, T_sat, Tin, Dh, Lh, G, PF):
    ...
    pipe = EffectivePipe(Lh, 1., 4*A/Dh, A)
...
def test_bpr_finite_nonzero_for_one_sided_channel():
    ...
    tin = mdot = 1.
    saved = chan.save(np.ones(len(chan)), T_left=5 * np.ones(n),
                      Tin=tin, mdot=mdot, p_abs=1e5)
    stat_p, power = saved['static_pressure'], saved['power']
    tsat = light_water.sat_temperature(stat_p)
    cpin = light_water.specific_heat(tin)
    bpr: np.ndarray = boiling_power(mdot, tsat, tin, cpin)
    extremes = {np.inf, -np.inf, np.nan, 0.}
