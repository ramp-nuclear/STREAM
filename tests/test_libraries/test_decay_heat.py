...
try:
    ...
    pytest.skip(
        "Standards of decay heat not available, skipping this file",
...
def test_decay_heat_contributions_are_monotonous_in_time():
    ...
    for contribution in decays:
        ...
        assert np.all(np.diff(decay_power) <= 0), f"{contribution} is not monotonous in time"
...
def test_fission_products_timed_contributions_against_known_values():
    ...
    assert np.allclose(fp_percent([0.0, hour, day, week]), [6.5, 1.3, 0.5, 0.3], rtol=1e-1)
...
def pos_med_arr(floats: SearchStrategy[float] = pos_medium_floats) -> SearchStrategy:
    return _lengths.flatmap(lambda n: st.lists(floats, min_size=n, max_size=n)).map(np.array)
...
def test_fission_decay_heat_is_constant_for_critical_core(L, ls, bs):
    ...
    assert np.allclose(
        fissions.profile(
            time=time,
            generation_time=L,
            delayed_groups_decay_rates=ls,
            delayed_neutron_fractions=bs,
            controls=ReactivityController(just(0.0)),
        )(time, np.inf),
        1.0,
    )
...
@settings(deadline=None)
@given(
    st.floats(1e-5, 1e-2, allow_nan=False, allow_infinity=False),
