...
@pytest.mark.parametrize(
    ("re", "epsilon", "turbulent_pd"),
    [(4.5e3, 0, 0.03847), (1e4, 0, 0.03087), (2e5, 0, 0.01565), (1e6, 0, 0.01165)],
)
...
@pytest.mark.parametrize(("a_dp", "K_R"), [(0.05, 0.71742), (0.1, 0.76565), (0.15, 0.81110), (0.2, 0.85346)])
...
def test_expansion_when_ratio_is_1_asserts_no_pressure(re):
    assert sudden_expansion_factor(1, re) == 0.0


@settings(deadline=None)
@given(normal_floats)
def test_contraction_when_ratio_is_1_asserts_no_pressure(re):
    assert sudden_contraction_factor(1, re) == 0.0
...
mdots = st.floats(-10, -1e-8, allow_nan=False, allow_subnormal=False) | st.floats(
    1e-8, 10, allow_nan=False, allow_subnormal=False
)
...
def test_idelchik_local_pd_computes_nonpositive_dp(a1, a2, mdot):
    ...
    dp = calc.dp_out(mdot=mdot, Tin=40.0)
...
def test_pressure_difference_for_a_few_simple_cases():
    ...
    pressure = partial(pressure_diff, fluid=light_water, pipe=pipe, dz=pipe.length, Tw=10000)

    assert np.isclose(pressure(T=20, mdot=0), light_water.density(20.0) * g * pipe.length)
...
def test_friction_factor_keyword_passing():
    args = T, Tw, mdot, fluid, pipe = 60.0, 80.0, 0.5, light_water, mock_pipe
    assert friction_factor("turbulent", epsilon=1.0)(*args) != friction_factor("turbulent", epsilon=0.0)(*args)
...
def test_regime_dependent_friction_assigns_regimes_correctly(re, md):
    ...
    if md < re_bounds[0]:
        ...
        assert np.allclose(f, lin_interp(*re_bounds, laminar_friction(md), turbulent_friction(md), md))
...
relative_curvature_out_of_range = st.floats(max_value=0.5, allow_nan=False, allow_subnormal=False, exclude_max=True)
