...
boundaries = st.lists(elements=reg_floats, min_size=2, max_size=40).map(np.array).map(np.cumsum)
...
b1 = delayed_groups.flatmap(lambda x: st.lists(reg_floats, min_size=x, max_size=x).map(np.array))
...
specific_calc_float_dicts = calc_float_dicts.flatmap(lambda d: st.fixed_dictionaries({key: reg_floats for key in d}))

channels = st.builds(calcs.Channel, z_boundaries=boundaries, fluid=light_water, pipe=pipes, name=names)
channels_and_contacts = st.builds(
    calcs.ChannelAndContacts,
...
res_sums = st.lists(resistors, min_size=1, max_size=50).map(lambda x: calcs.ResistorSum(*x))
