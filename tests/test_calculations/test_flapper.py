...
def test_closed_flapper_zero_flow_residue_is_zero(Tin, Tin_minus):
    F = Flapper(open_at_current=1.0, f=2.0, fluid=light_water, area=1,
                open_rate=1.0, stop_on_open=True)
    result = F.calculate([0, 0], mdot=0., Tin=Tin, Tin_minus=Tin_minus, t=0)

    assert result[F.indices("pressure")] == 0.
