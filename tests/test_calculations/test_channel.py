...
def test_channel_w_no_heat_flux_maintains_temperature_profile(T0, Tin, cells, time_points):
    """
...
mock_pipe = EffectivePipe(length=10, heated_perimeter=np.pi, wet_perimeter=np.pi, area=4 * np.pi)


@settings(deadline=None)
@given(
    integers(1, 10),
    floats(min_value=1.0, max_value=120.0, allow_nan=False, allow_infinity=False),
...
def test_channel_with_zero_flow_constant_h_reaches_wall_temperature(cells, T0, Tf, h_wall, fluid):
    """
    ...
    agr = Aggregator.from_decoupled(C, funcs={C: dict(mdot=0.0, T_left=Tf, T_right=Tf, p_abs=1e5, Tin=T0)})
    steady = agr.solve_steady({C.name: dict(T_cool=T0, pressure=1e5, h_left=h_wall, h_right=h_wall)})
...
def test_no_exceptions_raised_through_external_flow_inversion_on_Channel():
    ...
    agr = Aggregator.from_decoupled(
        C,
        funcs={C: dict(mdot=decreasing_mdot, T_left=80, T_right=80, Tin=80, p_abs=1e5)},
    )
