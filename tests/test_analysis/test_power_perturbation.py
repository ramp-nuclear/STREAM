...
decent_floats = st.floats(min_value=1e-5, max_value=1e3,
                          allow_nan=False, allow_infinity=False)
shifts = st.floats(min_value=0., max_value=0.99)
arrs = (arrays(float, st.tuples(st.integers(2, 5), st.integers(1, 5)),
               elements=decent_floats)
        .filter(lambda x: ~np.isclose(np.sum(x), np.max(np.sum(x, axis=1))))
        )
...
@given(arrs.filter(lambda x: np.any(np.diff(np.sum(x, axis=1)) != 0.)), shifts)
