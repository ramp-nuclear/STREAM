...
def _gen_records(key, value):
    yield from (
        [dict(i=0, j=0, calculation="foo", variable=key, value=value)]
        if key != "v"
        else [dict(i=0, j=j, calculation="moo", variable=key, value=v) for j, v in enumerate(value)]
    )
...
def test_simplemodel_returns_known_dataframe():
    df = _simplemodel(np.array([0.0, 0.0]), 2, 2)
    assert np.allclose(df.value.values, [4, 4, 4, 0, 0, 0])


def test_uq_of_simplemodel_by_known_result():
    model = UQModel({"vec": np.array([1.0, 2.0, 3.0]), "v1": 2.0, "v2": 3.0}, _simplemodel)
    sys, stat = model.uq(
        vec=Uncertainty(
            systematic_relative=np.array([0.1, 0.1, 0.1]),
            statistical_absolute=np.array([0.2, 0.2, 0.2]),
        ),
        v1=Uncertainty(systematic_absolute=1.0),
        v2=Uncertainty(statistical_absolute=1.0),
    )
    assert np.allclose(sys[:2], [1, 3])
    assert np.allclose(stat[0], 1.0)


non_negative = st.floats(min_value=0.0, max_value=3.0, allow_subnormal=False)
uncertainties = st.builds(Uncertainty, *(4 * [non_negative]))
positive = st.floats(min_value=1e-3, max_value=3.0)
one_positive = st.tuples(*(3 * [non_negative]), positive).flatmap(lambda tpl: st.permutations(tpl))
positive_uncertainty = one_positive.map(lambda tpl: Uncertainty(*tpl))


@given(uncertainties, positive_uncertainty)
def test_uq_of_multiple_sources_strictly_larger_than_one(u1: Uncertainty, u2: Uncertainty):
    model = UQModel({"vec": np.array([1.0, 2.0, 3.0]), "v1": 2.0, "v2": 3.0}, _simplemodel)
...
def test_uq_of_DASKUQModel_same_as_UQModel(u):
    params = {"vec": np.array([1.0, 2.0, 3.0]), "v1": 2.0, "v2": 3.0}
...
def test_dask_uq_graph_by_regression(persist, image_regression):
    params = {"vec": np.array([1.0, 2.0, 3.0]), "v1": 2.0, "v2": 3.0}
    dmodel = DASKUQModel(params, _simplemodel, _stupid_feature, _search_simple)
    taskgraph = dmodel.uq(v1=Uncertainty(1.0, 1.0, 3.0, 5.0), persist=persist)
    ext = "png"
...
def test_uq_improves_with_mdot_for_simple_channel():
    ...
    def _make_agr(mdot: float, tlow: float) -> tuple[Aggregator, State, np.array]:
        ...
        guess = State.merge(
            State.uniform(agr.graph, 20.0, "Tin", "T", "T_wall_left", "T_wall_right", "T_cool"),
            State.uniform(agr.graph, 0.0, "pressure"),
            {
                "Kirchhoff": {
                    "(In -> Out, 0)": mdot,
                    "(Out -> In, 0)": mdot,
    ...
    duq = [UQModel({"mdot": mdot, "tlow": 70.0}, _chf) for mdot in mdots]
    nominals = [uq.nominal for uq in duq]
    uqv = [
        uq.uq(
            mdot=Uncertainty(statistical_relative=0.05),
