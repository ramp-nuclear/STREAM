...
from stream.calculations import (
    Resistor, Pump, PointKinetics, PointKineticsWInput, Inertia,
    KirchhoffWDerivatives, Junction,
)
...
from stream.composition.subsystems import (
    point_kinetics_steady_state, symmetric_plate_steady_state,
)
...
def test_symmetric_plate_steady_state_has_zero_diff_in_power(Tin, iterations):
    ...
    state = symmetric_plate_steady_state(
        c=c, f=f, mdot=0.6, p_abs=2e5, power=1e4, Tin=Tin,
...
def test_symmetric_plate_steady_state_has_zero_diff_in_low_mdot(mdot):
    ...
    state = symmetric_plate_steady_state(
        c=c, f=f, mdot=mdot, p_abs=2e5, power=1e-6, Tin=35.,
...
def test_symmetric_plate_steady_state_accepts_negative_flow_rate():
    ...
    symmetric_plate_steady_state(c=c, f=f, mdot=mdot, p_abs=2e5, power=1e-6, Tin=35.)
...
def test_symmetric_plate_steady_state_solves_negative_flow_rate():
    ...
    assert np.isclose(T_top_computed, T_top_estimated, rtol=1.e-2)

lambdas = st.floats(1e-3, 10., allow_nan=False)
betas = st.floats(700 * pcm * 10., 700 * pcm * 1000., allow_nan=False)
lifetimes = st.floats(1e-6, 1e-4, allow_nan=False)
specific_dicts = st.fixed_dictionaries(dict(
...
powers = st.floats(min_value=1., max_value=3e7, allow_nan=False, allow_infinity=False)
...
def test_hydraulic_steady_state_is_a_root_for_a_single_loop_case():
    ...
    s = fg.guess_steady_state({r: 1., p: 1.}, 10)
...
def test_hydraulic_steady_state_is_a_root_for_a_simple_parallel_case():
    ...
    fg = FlowGraph(
        ...
        reference_node=(A, 3),
        abs_pressure_comps=[r, p])
    s = fg.guess_steady_state({r: 1., p: 1.5, r2: 0.5}, 10)
    y = fg.aggregator.load(s)
    assert np.allclose(fg.aggregator.compute(y), 0.0)


_FakeC = Calculation_factory(just(0.), [False, False], dict(Tin=0, pressure=1))
...
def test_hydraulic_steady_state_uses_strategy_when_provided():
    ...
    s = fg.guess_steady_state({r: 1., p: 1.}, 10, {fake: lambda mdot, T: mdot + T})
