...
calculations = st.integers(1, 5).flatmap(lambda n: st.builds(HasVariables, st.just(str(n)), *(n * [keys])))
calc_lists = st.lists(calculations, min_size=1).filter(lambda lst: len(set(c.name for c in lst)) == len(lst))
...
def test_uniform_sets_only_to_given_value(calcs, value, indices):
    ...
    calculated = reduce(operator.__or__, (set(subd.values()) for subd in d.values()))
...
def test_uniform_sets_variables_it_is_told_to(calcs, value, indices):
    ...
    d = State.uniform(calcs, value, *picked)
    calculated = reduce(operator.__or__, (set(subd.keys()) for subd in d.values()))
...
def test_merge_with_an_example():
    ...
    # noinspection PyTypeChecker
    assert State.merge(a, b) == {1: {2: 4, 3: 4, 4: 5}, 2: {"a": 1, "b": 2}, 3: {1: 2}}
